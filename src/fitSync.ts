...
import {
	...
	FileClash,
	FileStates,
	determineLocalChecksNeeded,
	resolveAllChanges,
	resolveUntrackedState,
} from "./util/changeTracking";
import type { LocalStores } from "@main";
...
function logCacheUpdate(
	...
	const localShaRemoved = Object.keys(oldLocalSha).filter(
		(k) => !newLocalSha[k],
	);
	...
	if (oldLocalCount > 0 && newLocalCount === 0) {
		warnings.push(
			`Local SHA cache dropped from ${oldLocalCount} to 0 files - possible data corruption`,
		);
	}
	...
	if (oldLocalCount === 0 && newLocalCount > 10) {
		warnings.push(
			`Local SHA cache jumped from 0 to ${newLocalCount} files - possible recovery from empty cache or first sync`,
		);
	}

	const totalChanges =
		localShaAdded.length +
		localShaRemoved.length +
...
export class FitSync implements IFitSync {
	...
	constructor(
		...
		this.onUserWarning =
			onUserWarning ??
			((msg) => fitLogger.log("[FitSync] User warning", { message: msg }));
	}
	...
	private async applyRemoteChanges(
		...
		const result = await this.fit.localVault.applyChanges(
			addToLocal,
			deleteFromLocal,
			{ clashPaths },
		);
	...
	private async compareAndResolveChanges(
		...
		detectNormalizationMismatches(
			Array.from(localScanPaths),
			Array.from(remoteScanPaths),
		);
		...
		for (const path of pathsNeedingShaCheck) {
			try {
				...
				fitLogger.log(
					`⚠️ [FitSync] Could not read file for SHA check: ${path}`,
					error,
				);
			}
		}
		...
		if (
			statError !== null ||
			filesMovedToFitDueToStatFailure.length > 0 ||
	...
	private async executeSync(
		...
		const deleteFromLocalNonClashed = safeRemote
			.filter((c) => c.type === "REMOVED")
			.map((c) => c.path);
		...
		const clashFiles = await Promise.all(
			clashes
				...
				.map(async (clash) => {
					const content = await this.fit.remoteVault.readFileContent(
						clash.path,
					);
		...
		if (pushResult && pushResult.pushedChanges.length > 0) {
			fitLogger.log(
				`.. ⬆️ [Push] Pushed ${pushResult.pushedChanges.length} changes to remote`,
			);
		}
		...
		if (
			addToLocalNonClashed.length > 0 ||
			deleteFromLocalNonClashed.length > 0 ||
			clashFiles.length > 0
		) {
		...
		if (Object.keys(newBaselineShas).length > 0) {
			fitLogger.log(
				"[FitSync] Updated local state with SHAs from written files",
				{
		...
		logCacheUpdate(
			...
			latestCommitSha,
			{
				localOpsApplied: localFileOpsRecord.changes.length,
				remoteOpsPushed: pushedChanges.length,
			},
		);
	...
	async sync(syncNotice: ISyncNotice): Promise<SyncResult> {
		...
		if (this.isSyncing) {
			fitLogger.log(
				"[FitSync] Sync already in progress - aborting new sync request",
			);
		...
		try {
			...
			const { changes: localChanges, state: currentLocalState } =
				localResult.value;
			const {
			...
			const filteredLocalChanges = localChanges.filter((c) =>
				this.fit.shouldSyncPath(c.path),
			);
			...
			if (localCount > 0 || remoteCount > 0) {
				...
				if (localCount > 0) {
					...
					["ADDED", "MODIFIED", "REMOVED"].forEach((changeType) => {
						const files = filteredLocalChanges
				...
				fitLogger.log(
					`🔄 [FitSync] Syncing changes (${localCount} local, ${remoteCount} remote)`,
					logData,
				);
			}
			...
			const { safeLocal, safeRemote, clashes, existenceMap } =
				await this.compareAndResolveChanges(
			...
			if (conflicts.length > 0) {
				...
				if (protectedConflicts.length > 0) {
					fitLogger.log(
						`[FitSync] ${protectedConflicts.length} path(s) blocked by sync policy`,
						{
			...
			return {
				success: false,
				error: SyncErrors.unknown(errorMessage, { originalError: error }),
			};
		} finally {
	...
	private async pushChangedFilesToRemote(
		...
		pushedChanges: FileChange[];
		lastFetchedRemoteSha: FileStates;
		lastFetchedCommitSha: CommitSha;
	} | null> {
		...
		for (const change of localUpdate.localChanges) {
			if (change.type === "REMOVED") {
				...
				if (existence !== "nonexistent") {
					fitLogger.log(
						"[FitSync] Skipping deletion - couldn't confirm local file actually deleted",
						{
							path: change.path,
							existence,
							reason:
		...
		const result = await this.fit.remoteVault.applyChanges(
			filesToWrite,
			filesToDelete,
			{ clashPaths: new Set() },
		);
		...
		if (result.changes.length === 0) {
			fitLogger.log(
				"[FitSync] No remote changes needed - content already matches",
				{
		...
		const pushedChanges = result.changes.map((op) => {
			const originalChange = localUpdate.localChanges.find(
				(c) => c.path === op.path,
			);
