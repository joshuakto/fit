...
> - Selective `.obsidian/` sync — a config file starts syncing once it exists in your GitHub repo (added there directly, or from another already-syncing device), and a `.fitattributes.json` at your vault root controls how it's handled. Some files (FIT's own settings, plugin installer files) are always excluded regardless.
